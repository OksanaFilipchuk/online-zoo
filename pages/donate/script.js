...
function changeMenuToBurger() {
  document
    .querySelector(".burger-background")
    .classList.toggle("burger-active");
  document.body.classList.toggle("hidden");
  document
    .querySelector(".burger-button")
    .classList.toggle("burger-button-active");
  document
    .querySelector(".burger-line1")
    .classList.toggle("burger-button-line1-active");
  document
    .querySelector(".burger-line2")
    .classList.toggle("burger-button-line2-active");
  document
    .querySelector(".burger-line3")
    .classList.toggle("burger-button-line3-active");
}
...
document.querySelector(".another-amount").addEventListener("input", (e) => {
  document
    .querySelector("#amount-25")
    .classList.remove("amount-25-non-highlight");

  if (e.target.value.length > 4) {
    document
      .querySelectorAll(".amount-input-non-highlight")
      .forEach((el) => el.classList.remove("amount-input-non-highlight"));
  ...
  if (index > -1) {
    ...
    if (document.querySelector(".amount-values-active")) {
      document
        .querySelector(".amount-values-active")
        .classList.remove("amount-values-active");
    }

    document
      .querySelector(".amount-values")
      .children[index].classList.add("amount-values-active");
  } else if (index === -1) {
    amounts.forEach((el) => {
      el.checked = "false";
    });
    document
      .querySelector("#amount-25")
      .classList.add("amount-25-non-highlight");
    if (document.querySelector(".amount-values-active")) {
      document
        .querySelector(".amount-values-active")
        .classList.remove("amount-values-active");
